 ...
 */
class BsFornecedoresFilter extends AbstractFilter
{
    ...
    public function __construct($serviceLocator = null)
    {
        ...
        // Informação para a coluna site:
        $site = new Input ( "site" );
        $site->setRequired ( true );
        $site->getFilterChain ()->attach ( $this->StringTrim );
        $site->getFilterChain ()->attach ( $this->StripTags );
        $site->getValidatorChain()->attach($this->emptyfilter);
        $this->add ( $site );
        
        // Informação para a coluna representante:
        $representante = new Input ( "representante" );
        $representante->setRequired ( true );
        $representante->getFilterChain ()->attach ( $this->StringTrim );
        $representante->getFilterChain ()->attach ( $this->StripTags );
        $representante->getValidatorChain()->attach($this->emptyfilter);
