 ...
 */
class ChangeColumnFilter extends InputFilter {
    ...
    public function __construct($serviceLocator = null) {
        ...
        // Informação para a coluna new_name:
        $new_name = new Input("new_name");
        $new_name->setRequired(true);
        $new_name->getFilterChain()->attach($StringTrim);
        $new_name->getFilterChain()->attach($StripTags);
        $new_name->getValidatorChain()->attach($emptyfilter);
        $this->add($new_name);
